...
from case_boss.converters.space_converter import SpaceCaseConverter


@pytest.fixture
def converter():
    return SpaceCaseConverter()
...
def test_space_case_converter(input_str, expected, converter):
...
def test_preserve_tokens():
    ...
    converter = SpaceCaseConverter(preserve_tokens=preserve_tokens)
    converter.convert(source=data)
    assert data == {
        "SQL alchemy": 1,
        "user ID": 1,
