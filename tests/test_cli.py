...
def test_transform_with_json_and_output_file(tmp_path):
    ...
    result = runner.invoke(
        app, ["transform", "--json", input_json, "--output", str(out_file)]
    )
    assert result.exit_code == 0
...
def test_transform_with_file_input(tmp_path):
    ...
    result = runner.invoke(
        app, ["transform", str(in_file), "--to", "camel", "--inplace"]
    )
    assert result.exit_code == 0
...
def test_transform_invalid_case_type():
    input_json = '{"simpleKey": 1}'
    result = runner.invoke(
        app, ["transform", "--json", input_json, "--to", "not_a_type"]
    )
...
def test_cli_preserve(tmp_path):
    ...
    result = runner.invoke(
        app,
        ["transform", str(input_file), "--to", "kebab", "--preserve", "SQL,HTTP,ID"],
    )
    assert result.exit_code == 0
...
def test_cli_exclude(tmp_path):
    ...
    result = runner.invoke(
        app,
        ["transform", str(input_file), "--to", "kebab", "--exclude", "metaData,test"],
    )
    assert result.exit_code == 0
...
def test_cli_transform_nested_dict_exclude_keys_stops_recursion(tmp_path):
    ...
    result = runner.invoke(
        app, ["transform", str(input_file), "--to", "kebab", "--exclude", "metaData"]
    )
...
def test_cli_transform_nested_dict_recursion_limit_1():
    input_data = '{"levelOne": {"levelTwo": {"levelThree": "value"}}}'
    result = runner.invoke(
        app, ["transform", "--json", input_data, "--to", "kebab", "--limit", "1"]
    )
...
def test_cli_transform_nested_dict_recursion_limit_2():
    input_data = '{"levelOne": {"levelTwo": {"levelThree": "value"}}}'
    result = runner.invoke(
        app, ["transform", "--json", input_data, "--to", "kebab", "--limit", "2"]
    )
